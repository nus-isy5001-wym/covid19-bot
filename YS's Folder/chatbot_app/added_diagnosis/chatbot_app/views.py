...
def webhook(request):
    ...
    if intent == "infection-status-covid":
        ...
        try:
            ...
            death_rate_ = pd_table[pd_table['country'] == country]['death_rate'].iloc[0]
    ...
    if intent == "diagnosis-covid":
        try:
            ...
            for i, prm in enumerate(param):
                value_str = req.get('queryResult').get('parameters').get(prm)
            ...
            if ((Q > 0) and (S > 0)) or ((Q==0) and (S==3)):
                text1 = res_list[0]['response'] 
            # High Risk 2
            elif q1 and (S==0):
                text1 = res_list[1]['response'] 
            # High Risk 3
            elif (not q1 and q2) and (S==0):
                text1 = res_list[2]['response'] 
            # Medium Risk 
            elif (Q==0) and (S==2):
                text1 = res_list[3]['response'] 
            # Low Risk 
            elif (Q==0) and (S==1):
                text1 = res_list[4]['response'] 
            # No Risk 
            elif (Q==0) and (S==0):
                text1 = res_list[5]['response'] 
            else:
                text1 = "Unknown response! Check for logics in Rule Base System."
        except "Invalid parameter value":
            text1 = "Parameter does not store either yes or no. Please check the entity naming in Dialogflow."
