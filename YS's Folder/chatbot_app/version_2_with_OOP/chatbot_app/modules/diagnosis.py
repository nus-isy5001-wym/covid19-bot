    ...
    def diagnosis(self):
        try:
            ...
            if ((Q > 0) and (S > 0)) or ((Q==0) and (S==3)):
                self.text1 = res_list[0]['response'] 
            # High Risk 2
            elif q1 and (S==0):
                self.text1 = res_list[1]['response'] 
            # High Risk 3
            elif (not q1 and q2) and (S==0):
                self.text1 = res_list[2]['response'] 
            # Medium Risk 
            elif (Q==0) and (S==2):
                self.text1 = res_list[3]['response'] 
            # Low Risk 
            elif (Q==0) and (S==1):
                self.text1 = res_list[4]['response'] 
            # No Risk 
            elif (Q==0) and (S==0):
                self.text1 = res_list[5]['response'] 
            else:
                self.text1 = "Unknown response! Check for logics in Rule Base System."
        except "Invalid parameter value":
            self.text1 = "Parameter does not store either yes or no. Please check the entity naming in Dialogflow."
        finally:
